...
fn dict_create_and_get() {
    ...
    let result = interp.eval("dict get [dict create name alice age 30] name").unwrap();
...
fn dict_exists_true() {
    let mut interp = Interpreter::new();
    let result = interp.eval("dict exists [dict create name alice] name").unwrap();
...
fn dict_exists_false() {
    let mut interp = Interpreter::new();
    let result = interp.eval("dict exists [dict create name alice] missing").unwrap();
...
fn dict_keys() {
    let mut interp = Interpreter::new();
    let result = interp.eval("dict keys [dict create name alice age 30]").unwrap();
...
fn dict_values() {
    let mut interp = Interpreter::new();
    let result = interp.eval("dict values [dict create name alice age 30]").unwrap();
...
fn dict_size() {
    let mut interp = Interpreter::new();
    let result = interp.eval("dict size [dict create name alice age 30]").unwrap();
...
fn dict_get_missing_key_error() {
    let mut interp = Interpreter::new();
    let err = interp.eval("dict get [dict create name alice] missing").unwrap_err();
    let msg = format!("{}", err);
    assert!(msg.contains("not known"), "expected 'not known' in error, got: {msg}");
}
